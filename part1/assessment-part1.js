...
var Vehicle = function() {
  var gasRemaining = 100;
  this.drive = function(gasRemaining) {
    this.gasRemaining -= 25;
  };
  this.drive();
  console.log(gasRemaining);
};
...
String.prototype.grammarPolice = function(str) {
  ...
  for (var i=0; i<str.length; i++) {
    if (str[i].match(/[a-z]/i) && str[i-1] === " ") {
      grammarized.push(str[i].toUpperCase);
    } else if (str[i] !== (/[a-z]/i)) {
      grammarized.push(str[i]);
    } else {
      grammarized.push(str[i].toLowerCase);
    }
  }
  return grammarized;
  console.log(grammarized);
}
...
function valueType(paramOne, paramTwo) {
  if (typeof(paramOne) === typeof(paramTwo)) {
